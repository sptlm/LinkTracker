...
public class GithubLinkUpdater implements LinkUpdater {
    ...
    private String formatDescription(GithubIssueItem item) {
        ...
        String author = item.user() != null ? MessageFormattingUtils.safe(item.user().login()) : "unknown";
        String createdAt = MessageFormattingUtils.formatInstant(item.createdAt());
        String preview = MessageFormattingUtils.truncateWithEllipsis(MessageFormattingUtils.safe(item.body()), PREVIEW_LENGTH);
