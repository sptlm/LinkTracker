...
public class SqlSubscriptionRepository implements SubscriptionRepository {
    ...
    public java.util.Optional<LinkSubscription> findByChatIdAndLinkId(long chatId, long linkId) {
        return jdbcClient.sql(baseQuery() + " where s.chat_id = :chatId and s.link_id = :linkId group by s.chat_id, s.link_id, s.created_at")
                .param("chatId", chatId)
                .param("linkId", linkId)
    ...
    public List<LinkSubscription> findByChatId(long chatId) {
        return jdbcClient.sql(baseQuery() + " where s.chat_id = :chatId group by s.chat_id, s.link_id, s.created_at order by s.link_id")
                .param("chatId", chatId)
                .query(SUBSCRIPTION_ROW_MAPPER)
                .list();
    }
    ...
    public List<LinkSubscription> findByLinkId(long linkId) {
        return jdbcClient.sql(baseQuery() + " where s.link_id = :linkId group by s.chat_id, s.link_id, s.created_at order by s.chat_id")
                .param("linkId", linkId)
                .query(SUBSCRIPTION_ROW_MAPPER)
                .list();
    }
    ...
    public List<Long> findChatIdsByLinkId(long linkId) {
        return jdbcClient.sql("select chat_id from link_subscription where link_id = :linkId order by chat_id")
                .param("linkId", linkId)
                .query(Long.class)
                .list();
    }
    ...
    public void delete(long chatId, long linkId) {
        jdbcClient.sql("delete from link_subscription where chat_id = :chatId and link_id = :linkId")
                .param("chatId", chatId)
                .param("linkId", linkId)
                .update();
    }
