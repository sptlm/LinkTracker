...
class RateLimitingIT extends AbstractPostgresIT {
    ...
    void shouldReturnTooManyRequestsWhenChatLimitExceeded() throws Exception {
        mockMvc.perform(post("/tg-chat/1").header(TG_CHAT_ID_HEADER, "1").with(request -> {
                    request.setRemoteAddr("203.0.113.10");
                    return request;
                }))
                .andExpect(status().isOk());

        mockMvc.perform(post("/tg-chat/2").header(TG_CHAT_ID_HEADER, "1").with(request -> {
    ...
    void shouldUseSeparateLimitsForDifferentChatsBehindSameBotInstance() throws Exception {
        ...
        mockMvc.perform(post("/tg-chat/2").header(TG_CHAT_ID_HEADER, "2").with(request -> {
                    request.setRemoteAddr("203.0.113.10");
                    return request;
                }))
                .andExpect(status().isOk());
